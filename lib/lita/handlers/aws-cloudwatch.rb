module Lita
  module Handlers
    class AwsCloudWatchHandler < AwsBaseHandler
      ...
      route(/aws ec2\-memutil ([i][\-][0-9a-zA-Z]+)[ ]*(.*)$/, help: help, restrict_to: protect ) do |response|
      ...
      route(/aws ec2\-cpuutil ([i][\-][0-9a-zA-Z]+)[ ]*(.*)$/, help: help, restrict_to: protect) do |response|
      ...
      route(/aws elb\-req-sum ([0-9a-zA-Z_]+)[ ]*(.*)$/, help: help, restrict_to: protect) do |response|
        opts = get_options_for_cloudwatch(response, ago: '2d', period: '300s')
      ...
      route(/aws rds\-space ([0-9a-zA-Z_]+)[ ]*(.*)$/, help: help, restrict_to: protect) do |response|
