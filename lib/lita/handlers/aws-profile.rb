module Lita
  module Handlers
    class AwsProfile < AwsBaseHandler
      ...
      protect = ['aws_admin', 'aws_profile']
      help = { 'aws profile' => 'list all aws profile.' }
      route(/aws profile$/, help: help, restrict_to: protect) do |response|
      ...
      route(/aws profile ([a-zA-Z\-0-9]+) ([a-z\-0-9]+) ([^ ]+) ([^ ]+)/, help: help, restrict_to: protect) do |response|
      ...
      protect = ['aws_admin', 'aws_profile', 'aws_profile_describe']
      help = { 'aws account-id[ --profile NAME]' => 'Get your aws account id.' }
      route(/aws account\-id[ ]*(.*)$/, help: help, restrict_to: protect) do |response|
